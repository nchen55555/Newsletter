...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Similarity service responded with status ${response.status}: ${errorText}`);
    }

    const data = await response.json();
    
    // Add runtime info to distinguish this specific endpoint
    data.runtime = 'nextjs-candidate-similarity';
    data.proxied_from = 'python-flask';
    data.endpoint_type = 'candidate-similarity';
    
    return NextResponse.json(data);

  } catch (error) {
    console.error('Candidate similarity API error:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to connect to similarity service',
      details: error instanceof Error ? error.message : String(error),
      runtime: 'nextjs-candidate-similarity',
      endpoint_type: 'candidate-similarity'
    }, { status: 500 });
  }
}
...
export async function GET() {
  try {
    ...
    data.proxied_from = 'python-flask';
    data.endpoint_type = 'candidate-similarity';
    data.message = 'Candidate similarity calculation endpoint';
    
    return NextResponse.json(data);

  } catch (error) {
    console.error('Candidate similarity API GET error:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to connect to similarity service',
      details: error instanceof Error ? error.message : String(error),
